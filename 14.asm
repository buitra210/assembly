.data
str1: .space MAX_LEN     # Chu?i 1
str2: .space MAX_LEN     # Chu?i 2
dem1: .space 26          # M?ng t?n su?t 1
dem2: .space 26          # M?ng t?n su?t 2
commonchars: .word 0     # S? ký t? chung

.text
.globl main
main:
    # ??c chu?i 1 t? bàn phím
    li $v0, 4
    la $a0, prompt1
    syscall
    li $v0, 8
    ...
    # Tính t?n su?t xu?t hi?n c?a ký t? trong chu?i 1
    la $t0, str1
    lb $t1, 0($t0)
    beqz $t1, count_done1
    count1_loop:
    ...
    # Tính t?n su?t xu?t hi?n c?a ký t? trong chu?i 2
    la $t0, str2
    lb $t1, 0($t0)
    beqz $t1, count_done2
    count2_loop:
